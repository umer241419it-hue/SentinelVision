...
echo "=== INVOKE WRITE: CreateAsset assetSentinel3 ==="
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "$CORE_PEER_TLS_ROOTCERT_FILE" --peerAddresses localhost:9051 --tlsRootCertFiles "$ORG2_CA" -c '{"function":"CreateAsset","Args":["assetSentinel3","sapphire","35","MaladSentinelUnit3","9500"]}'

sleep 3

echo "=== QUERY READ: assetSentinel3 ==="
peer chaincode query -C mychannel -n basic -c '{"Args":["ReadAsset","assetSentinel3"]}'
